...
//Найти сумму отрицательных двухзначных элементов или выдать сообщение, что таких элементов нет.
#include <iostream>
#include <fstream>
#include <string>
using namespace std;
...
void menu()
{
    ...
    cout << " 7 - Найти сумму отрицательных двухзначных элементов" << endl;
    cout << " 8 - Выход" << endl;
    cout << "Сделайте выбор: ";
}
...
void find_negative_two_digit_sum(list* p) {
    int sum = 0;
    bool found = false;
    while (p != nullptr) {
        if (p->a < 0 && p->a > -100 && p->a < -9) {
            sum += p->a;
            found = true;
        }
        p = p->next;
...
int main()
{
    ...
    while (choice != 8) {
        if (choice == 1) {
            ...
            find_negative_two_digit_sum(plist);
        }
        menu();
        cin >> choice;
    }
